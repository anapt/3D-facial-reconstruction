...
def read_dfs():
    ...
    data_mse = pd.read_csv("./EVALUATION/{}_loss.csv".format('mse'))

    resnet = data_resnet.values
    data_mse.insert(1, "resnet50_loss", resnet, True)
    print(data_mse.head())

    count = 0

    data_mse.to_csv(r'./{}.csv'.format('compare_loss_functions'), index=None, header=True)
...
data = pd.read_csv("./EVALUATION/{}.csv".format('compare_loss_functions'))
print(data.to_latex(index=True))


def bar_plot():
    data = pd.read_csv("./EVALUATION/{}.csv".format('compare_loss_functions'))
    mean_mse = np.mean(data['mse_loss'])
    mean_resnet = np.mean(data['resnet50_loss'])

    means = (mean_resnet, mean_mse)
    print(means)
    N = 2
    plt.figure(figsize=(16, 9))
    ind = np.arange(N)  # the x locations for the groups
    width = 0.40  # the width of the bars: can also be len(x) sequence
    std = (np.std(data['resnet50_loss']), np.std(data['mse_loss']))
    print(std)
    p1 = plt.barh(ind, means, width, xerr=std, tick_label=['Custom Loss', 'Mean Squared Error'],
                  color=['peachpuff', 'lavender'])
