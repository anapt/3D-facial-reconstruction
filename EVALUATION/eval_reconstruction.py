...
# get_reconstructions(arch='resnet101')
...
def read_dfs():
    ...
    data_resnet101 = pd.read_csv("./EVALUATION/{}_loss.csv".format('resnet101'))
    ...
    resnet = data_resnet101.values
    # inception = data_inception.values
    data_resnet.insert(1, "resnet101_loss", resnet, True)
    # data_xception.insert(2, "inceptionV3_loss", inception, True)
    print(data_resnet.head())

    data_resnet.to_csv(r'./EVALUATION/{}_losses.csv'.format('resnet_archs'), index=None, header=True)
...
def bar_plot():
    data = pd.read_csv("./EVALUATION/{}.csv".format('resnet_archs_losses'))
